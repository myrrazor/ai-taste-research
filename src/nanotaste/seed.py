...
def _seed_url(workspace: TasteWorkspace, url: str, domain: str, label: str | None) -> SeedRecord:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise ValueError("seed URL must be http or https")
    request = Request(url, headers={"User-Agent": "NanoTaste/0.1 (+https://github.com/myrrazor/ai-taste-research)"})
    try:
        with urlopen(request, timeout=12) as response:  # noqa: S310 - scheme already restricted
    ...
    return SeedRecord("url", chosen, label or parsed.netloc, excerpt, url, None)
...
def _seed_path(
    ...
    dest = workspace.seed_files_dir / path.name
